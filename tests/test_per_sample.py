"""Per-sample DataFrame + throughput diagnostics (Milestone 2)."""
...
def test_per_sample_shape_and_columns():
    model, loader = _setup()
    r = Evaluator(model, loader, [metrics.Accuracy(), metrics.F1()], "cpu").run()
...
def test_per_sample_accuracy_mean_equals_aggregate():
    model, loader = _setup()
    r = Evaluator(model, loader, [metrics.Accuracy()], "cpu").run()
